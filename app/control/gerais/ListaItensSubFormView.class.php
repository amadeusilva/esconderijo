 ...
 */
class ListaItensSubFormView extends TPage
{
    private $form; // form
    private $embedded;
    ...
    function __construct($param, $embedded = false)
    {
        ...
        // creates the form
        $this->form = new BootstrapFormBuilder('form_lista_itens_sub');
        if (!$this->embedded) {
            $this->form->setFormTitle('Lista de Subitens');
        }
        $this->form->setClientValidation(true);

        // create the form fields
        $id                = new TEntry('id');
        $lista_itens_id    = new TDBCombo('lista_itens_id', 'adea', 'ListaItens', 'id', '{item} ({Lista->lista})', 'id');
        ...
        $this->form->addFields([new TLabel('Lista de Itens')],   [$lista_itens_id]);
        ...
        $lista_itens_id->addValidation('Lista de Itens', new TRequiredValidator);
    ...
    public static function onSave($param)
    {
        ...
        try {
            ...
            $listaitensid = new ListaItensSub();
            $listaitensid->fromArray($param);
            ...
            $data = new stdClass;
            $data->id = $listaitensid->id;
            TForm::sendData('form_lista_itens_sub', $data);

            if (!$param['embedded']) {
                TScript::create("Template.closeRightPanel()");

                $posAction = new TAction(array('ListaItensSubDataGridView', 'onReload'));
                $posAction->setParameter('target_container', 'adianti_div_content');

                // shows the success message
                new TMessage('info', 'Record saved', $posAction);
            } else {
                TWindow::closeWindowByName('ListaItensSubFormView');
            }
    ...
    function onEdit($param)
    {
        try {
            if (isset($param['id'])) {
                ...
                $listaitensid = new ListaItensSub($param['id']);
                ...
                $this->form->setData($listaitensid);
