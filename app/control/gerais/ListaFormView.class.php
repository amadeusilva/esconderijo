 ...
 */
class ListaFormView extends TPage
{
    private $form; // form
    private $embedded;
    ...
    function __construct($param, $embedded = false)
    {
        ...
        $this->form->addFields([new TLabel('Descrição')],   [$lista]);
        ...
        $lista->addValidation('Descrição', new TRequiredValidator);
    ...
    public static function onSave($param)
    {
        ...
        try {
            ...
            $data = new stdClass;
            $data->id = $lista->id;
            TForm::sendData('form_lista', $data);

            if (!$param['embedded']) {
                TScript::create("Template.closeRightPanel()");

                $posAction = new TAction(array('ListaDataGridView', 'onReload'));
                $posAction->setParameter('target_container', 'adianti_div_content');

                // shows the success message
                new TMessage('info', 'Record saved', $posAction);
            } else {
                TWindow::closeWindowByName('ListaFormView');
            }
