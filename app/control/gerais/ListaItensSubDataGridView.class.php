 ...
 */
class ListaItensSubDataGridView extends TPage
{
    ...
    public function __construct()
    {
        ...
        $this->setActiveRecord('ListaItensSub'); // defines the active record
        $this->setDefaultOrder('id', 'asc');  // defines the default order
        $this->addFilterField('id', '=', 'id'); // add a filter field
        $this->addFilterField('lista_itens_id', '=', 'lista_itens_id'); // add a filter field
        ...
        $col_lista_itens_id  = new TDataGridColumn('{ListaItens->abrev} - ({ListaItens->Lista->lista})', 'Lista', 'rigth');
        ...
        $col_lista_itens_id->setAction(new TAction([$this, 'onReload']), ['order' => 'lista_itens_id']);
        ...
        $this->datagrid->addColumn($col_lista_itens_id);
        $this->datagrid->addColumn($col_item);
        $this->datagrid->addColumn($col_abrev);
        $this->datagrid->addColumn($col_obs);
        
        // creates two datagrid actions
        $action1 = new TDataGridAction(['ListaItensSubFormView', 'onEdit'], ['id'=>'{id}', 'register_state' => 'false']);
        ...
        // creates the form
        $this->form = new TForm('form_search_lista_itens_sub');
        ...
        $id               = new TEntry('id');
        $lista_itens_id   = new TDBCombo('lista_itens_id', 'adea', 'ListaItens', 'id', '{item} ({abrev})', 'id');
        $lista_itens_id->enableSearch();
        ...
        // avoid focus on tab
        $id->tabindex = -1;
        $lista_itens_id->tabindex = -1;
        $item->tabindex = -1;
        $abrev->tabindex = -1;
        $obs->tabindex = -1;
        
        $id->setExitAction( new TAction([$this, 'onSearch'], ['static'=>'1']) );
        $lista_itens_id->setChangeAction( new TAction([$this, 'onSearch'], ['static'=>'1']) );
        ...
        $panel = new TPanelGroup('Listas de Subitens');
        $panel->add($this->form);
        ...
        $panel->addHeaderActionLink('Novo',  new TAction(['ListaItensSubFormView', 'onEdit'], ['register_state' => 'false']), 'fa:plus green' );
