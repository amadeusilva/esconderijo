...
class PalestranteDataGrid extends TPage
{
    ...
    public function __construct()
    {
        ...
        $this->addFilterField('palestra_id', '=', 'palestra_id'); // filterField, operator, formField
        ...
        // creates the form
        $this->form = new BootstrapFormBuilder('form_search_PalestranteDataGrid');
        ...
        $this->form->addFields([new TLabel('Palestra')],      [$palestra_id]);
        ...
        $this->form->setData(TSession::getValue('PalestranteDataGrid_filter_data'));
        ...
        $col_equipe->setTransformer(function ($value, $objeto) {
            ...
            if ($objeto) {
                //$icon  = "<i class='far fa-envelope' aria-hidden='true'></i>"; //{$icon} 
                return "<a generator='adianti' href='index.php?class=PalestranteDataGrid&method=onViewDetalhesPalestra&tipo=2&encontro_id=$objeto->encontro_id&equipe_id=$objeto->palestra_id&encontro=$objeto->encontro&equipe=$objeto->palestra&casal_id=$objeto->casal_id'>$value</a>";
            }
            return $value;
        });
    ...
    public static function onViewDetalhesPalestra($param)
    {
        try {
            ...
            $win = TWindow::create('Detalhes da Palestra', 0.6, 0.8);
