...
class EncontroPanel extends TPage
{
    ...
    public function onView($param)
    {
        try {
            ...
            $total_encontristas = ViewEncontrista::where('encontro_id', '=', $encontro->id)->countDistinctBy('casal_id');
            $total_encontreiros = ViewEncontreiro::where('encontro_id', '=', $encontro->id)->countDistinctBy('casal_id');
            ...
            if ($encontreiros) {
                ...
                foreach ($encontreiros as $encontreiro) {
                    ...
                    // adds a row to the table
                    $row = $table->addRow();
                    $row->style = 'font-weight: bold; border-bottom: 2px solid black; background-color: red;';
                    $title = $row->addCell($title);
                    $title->colspan = 6;
