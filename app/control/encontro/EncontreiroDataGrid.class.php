...
class EncontreiroDataGrid extends TPage
{
    ...
    public function __construct()
    {
        ...
        $this->addFilterField('equipe_id', '=', 'equipe_id'); // filterField, operator, formField
        ...
        // creates the form
        $this->form = new BootstrapFormBuilder('form_search_EncontreiroDataGrid');
        $this->form->setFormTitle('Lista de Encontreiros (Filtros)');
        ...
        $equipe_id = new TDBCombo('equipe_id', 'adea', 'Equipe', 'id', '{equipe} ({id})', 'id');
        ...
        $this->form->addFields([new TLabel('Equipe')],      [$equipe_id]);
        ...
        $this->form->setData(TSession::getValue('EncontreiroDataGrid_filter_data'));
        ...
        $col_circulo = new TDataGridColumn('CirculoCor', 'Círculo', 'center');
        ...
        $col_equipe->setTransformer(function ($value, $objeto) {
            ...
            if ($objeto) {
                //$icon  = "<i class='far fa-envelope' aria-hidden='true'></i>"; //{$icon} 
                return "<a generator='adianti' href='index.php?class=EncontreiroDataGrid&method=onViewDetalhesEquipe&tipo=1&encontro_id=$objeto->encontro_id&equipe_id=$objeto->equipe_id&encontro=$objeto->encontro&equipe=$objeto->equipe&casal_id=$objeto->casal_id'>$value</a>";
            }
            return $value;
        });
        ...
        $col_camisa_encontro_cor->enableAutoHide(1000);
        $col_disponibilidade_nt->enableAutoHide(1000);
        $col_coordenar_s_n->enableAutoHide(1000);
        ...
        $action1 = new TDataGridAction(['EncontreiroForm', 'onEdit'],   ['key' => '{id}', 'tipo_enc_id' => 1, 'register_state' => 'false']);
        $action2 = new TDataGridAction([$this, 'onDelete'], ['key' => '{id}', 'encontro_id' => '{encontro_id}', 'casal_id' => '{casal_id}', 'equipe_id' => '{equipe_id}']);
    ...
    public static function onViewDetalhesEquipe($param)
    {
        try {
            ...
            if ($param['tipo'] == 1) {
                ...
                $encontreiro_equipe = ViewPalestrante::where('encontro_id', '=', $param['encontro_id'])->where('palestra_id', '=', $param['equipe_id'])->orderBy('funcao_id, casal', 'asc')->load();
                $nome_desc = 'Palestra';
            } else if ($param['tipo'] == 3) {
                $encontreiro_equipe = ViewEdg::where('encontro_id', '=', $param['encontro_id'])->where('pasta_id', '=', $param['equipe_id'])->orderBy('funcao_id, casal', 'asc')->load();
                $nome_desc = 'Pasta';
            }

            $win = TWindow::create('Detalhes da Equipe', 0.6, 0.8);
